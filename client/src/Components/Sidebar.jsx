          ...
          {filteredUsers.map((user,index)=>(
            <div onClick={()=>{setSelectedUser(user);setUnseenMessages(prev=>({...prev,[user._id]:0}))}} key={index}className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===user._id && 'bg-[#282142]/50'}`}>
              ...
              <div className='flex flex-col leading-5'>
                <p>{user.fullName}</p>
                {/* {
                  onlineUsers.s(user._id)?<span className='text-green-400 text-xs'>Online</span>:<span className='text-neutral-400 text-xs'>Offline</span>
                } */}
              </div>
              {unseenMessages[user._id] >0 && <p className='absolute top-4 right-4 text-x5 h-5 w-5 flex justify-center rounded-full bg-violet-500/50'>{unseenMessages[user._id]}</p>}
              </div>
          ))}
