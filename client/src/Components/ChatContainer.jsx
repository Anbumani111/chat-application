...
//           <label htmlFor="image">
//             <img  onClick={handleSendImage} src={assets.gallery_icon} alt="" className='w-5 mr-2 cursor-pointer'/>

//           </label>
       
//        </div>
//        <img onChange={handleSendMessage} src={assets.send_button} alt="" className='w-7 cursor-pointer' />
//       </div>
...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full relative overflow-hidden backdrop-blur-lg'>
      ...
      {/* HEADER */}
      <div className='flex items-center gap-3 py-3 px-4 border-b border-stone-600'>
        <img
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-auto p-3 pb-6'>
        ...
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`flex items-end gap-2 mb-2 ${msg.senderId === authUser._id ? 'justify-end' : 'justify-start'}`}
          >
            ...
            {msg.image ? (
              <img
                src={msg.image}
                alt=""
                className='max-w-[220px] rounded-lg border border-gray-700'
              />
            ) : (
              <p className={`p-2 max-w-[220px] text-sm rounded-lg break-words 
    ...
    <div className='flex flex-col items-center justify-center gap-3 text-gray-400 bg-white/5 max-md:hidden h-full'>
